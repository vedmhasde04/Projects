...
class MarvellousLogin extends Template implements ActionListener, Runnable{
  ...
  MarvellousLogin(){
    ...
    TopLabel.setText("Marvellous Packe Unpacker : Login");
    ...
    text1.setToolTipNext("Enter USERNAME");
    ...
    text2.setToolTipNext("ENTER PASSWORD");
  ...
  public void actionPerformed(ActionEvent ae){
    ...
    if(ae.getSource() == SUBMIT){
      if(Validate(value1,value2) == false){
        text1.setText("");
        text2.setText("");
        JOption.showMessageDialog(this,"Short Username","Marvellous Packer Unpacker",JOptionPane.ERROR_MESSAGE);
    }
    ...
    for(; ;){
      if(text2.isFocusOwner()){
        if(Toolkit.getDefaultToolKit().getLockingKeyState(KeyEvent.VK_CAPS_LOCK)){
          text2.setToolTipNext("Warning: CAPS LOCK is on");
        }

        else
          text2.setToolTipNext("");
